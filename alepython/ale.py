# From https://github.com/blent-ai/ALEPython
import numpy as np
...
def _ax_quantiles(ax, quantiles, twin='x'):
    ...
    if twin == 'x':
        ...
        ax_top.set_xticklabels(["{1:0.{0}f}%".format(int(i / (len(quantiles) - 1) * 100 % 1 > 0), i / (len(quantiles) - 1) * 100) for i in range(len(quantiles))], color="#545454", fontsize=5)
        ax_top.set_xlim(ax.get_xlim())
    elif twin =='y':
        ax_right = ax.twinx()
        ax_right.set_yticks(quantiles)
        ax_right.set_yticklabels(["{1:0.{0}f}%".format(int(i / (len(quantiles) - 1) * 100 % 1 > 0), i / (len(quantiles) - 1) * 100) for i in range(len(quantiles))], color="#545454", fontsize=5)
...
def ale_plot(model, train_set, features, title=None, figsize=None, bins=10, monte_carlo=False, predictor=None, features_classes=None, **kwargs):
    ...
    if len(features) == 1:
        ...
        if features_classes is None:
            ...
            _ax_labels(fig.gca(), features[0], features[1])
